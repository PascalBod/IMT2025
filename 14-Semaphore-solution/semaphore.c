...
/*
    RTOS for ML presentation - solution to exercise 14.
...
void task_code(void *pvParameters) {
    ...
    // First, wait.
    vTaskDelay(pdMS_TO_TICKS(paramsPtr->wait_time_ms));

    // Then, try to take the semaphore.
    app_log_info("%lu - Requesting to take the semaphore\n", id);
    BaseType_t os_rs = xSemaphoreTake(paramsPtr->semaphore,
                                      pdMS_TO_TICKS(SEM_MAX_WAIT_PERIOD_MS));
    if (os_rs != pdTRUE) {
        app_log_error("%lu - Error from xSemaphoreTake\n", id);
        reset_on_fatal_error();
    }
    app_log_info("%lu - Semaphore taken\n", id);

    // Perform our job.
    vTaskDelay(pdMS_TO_TICKS(TASK_WORKING_TIME_MS));

    // Give the semaphore.
    os_rs = xSemaphoreGive(paramsPtr->semaphore);
    if (os_rs != pdTRUE) {
        app_log_error("%lu - Error from xSemaphoreGive\n", id);
        reset_on_fatal_error();
    }
