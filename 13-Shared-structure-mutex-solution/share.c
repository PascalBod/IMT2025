...
/*
    RTOS for ML presentation - solution to exercise 13.
...
#define MUTEX_WAIT_PERIOD_MS    300
...
static void task1_code(void *arg) {
  ...
  while (true) {
      os_rs = xSemaphoreTake(mutex, pdMS_TO_TICKS(MUTEX_WAIT_PERIOD_MS));
      if (os_rs != pdTRUE) {
          app_log_warning("Task1 - Couldn't acquire the mutex\n");
          continue;
      }
      if (a == 1) {
          // Simulate some processing.
      ...
      os_rs = xSemaphoreGive(mutex);
      if (os_rs != pdTRUE) {
          app_log_warning("Task1 - Couldn't release the mutex\n");
          continue;
      }
      if (b != 1) {
...
static void task2_code(void *arg) {
  ...
  while (true) {
      ...
      os_rs = xSemaphoreTake(mutex, pdMS_TO_TICKS(MUTEX_WAIT_PERIOD_MS));
      if (os_rs != pdTRUE) {
          app_log_warning("Task2 - Couldn't acquire the mutex\n");
          continue;
      }
      a = 2;
      os_rs = xSemaphoreGive(mutex);
      if (os_rs != pdTRUE) {
          app_log_warning("Task2 - Couldn't release the mutex\n");
          continue;
      }
...
static void task3_code(void *arg) {
  ...
  while (true) {
      ...
      os_rs = xSemaphoreTake(mutex, pdMS_TO_TICKS(MUTEX_WAIT_PERIOD_MS));
      if (os_rs != pdTRUE) {
          app_log_warning("Task3 - Couldn't acquire the mutex\n");
          continue;
      }
      a = 1;
      os_rs = xSemaphoreGive(mutex);
      if (os_rs != pdTRUE) {
          app_log_warning("Task3 - Couldn't release the mutex\n");
          continue;
      }
  }
...
void share_init(void) {
  ...
  if (mutex == NULL) {
      app_log_error("Error from xSemaphoreCreateMutex\n");
