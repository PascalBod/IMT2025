...
#define SIM_PROCESS_TIME_PERIOD_MS 800
...
static void producer_code(void *pvParameters) {
  ...
  while (true) {
      ...
      app_log_info("Producer %u - Writing %lu into queue\n",
                   task_args->producer_id, counter);
      os_rs = xQueueSendToBack(queue, &message, 0);
      if (os_rs == errQUEUE_FULL) {
          app_log_warning("Producer %u - Queue full, counter lost\n",
                          task_args->producer_id);
      }
      counter++;
...
static void consumer_code(void *pvParameters) {
  ...
  while (true) {
      ...
      vTaskDelay(pdMS_TO_TICKS(SIM_PROCESS_TIME_PERIOD_MS));
  }
